...
| RT-004 | Phase-1 廉价筛查 (r20+GBDT-on-emb) | **done** | **no_residual** (残差IC=-0.0026 t=-0.6, 廉价REJECT) |
| RT-005 | walk-forward gate 四臂 | **skip** | **REJECT** (precondition: RT-004 no_residual) |
| RT-006 | opt-in 落地 (依赖 RT-005=PASS) | skip | skipped (未PASS不落地) |

## 终局结论 (2026-06-04, 循环完结)
**关系张量 v2 (CNN+TCN 多尺度软符号 K 线关系编码) = 标准 TA 标量版换皮, REJECT 不落地。**
Phase-1 廉价筛查给出决定性证据: 张量自身对 r20 的 raw rank-IC ≈ -0.0009 (t=-0.18, 形同零),
A2 融合 (张量+A0) = 0.10103 ≈ A0 标量TA = 0.10114 (**零增量**); 扣 A0+月×板块后残差 rank-IC=-0.0026
(t=-0.6, 904K OOF 样本/182 日), 远未达事前注册阈值 |IC|>=0.01 且 |t|>=3。分regime 动量 +0.011/反转 -0.012
符号相反互相抵消 (无稳健方向)。**先验完全兑现** (设计 §6: 大头≈标准TA, CNN 非线性形态交互的真增量小到不存在)。
这是关系张量/梅花同类陷阱的第 8 个被反过拟合脚手架诚实否决的假设 (廉价 REJECT, 省 RT-005 walk-forward ~90% 算力)。
生产线 V12.31 全程冻结未触碰。
...
- 迭代4 | RT-004 (+ precondition 级联 RT-005/006) | Phase-1 廉价筛查落地并给出**终局 no_residual REJECT**。**方法**: research/rt004_phase1_screen.py 跑 3 折 expanding-window OOF (f1 train202206-06/test202307-09, f2→test202310-12, f3→test202401-03; **全部 < 202410 gate 窗以保 RT-005 OOS 纯净 SIGN-R02**), 复用 rt003_train 低层件 (gather_batch/RT_FEATS/A0_COLS/N_SEQ=40)。每折: 训 FusionModel(NN end-to-end, 2 epoch, train 下采样 40k) → 冻结 SeqEncoder 抽序列 embedding(64) → ① emb_pred=GBDT(emb→r20z) (张量自身≈A1) ② a0_pred=GBDT(A0 32标量→r20z) (baseline) ③ a2_pred=GBDT(emb⊕A0→r20z) (融合描述)。**决策指标**=emb_pred 与 r20 同对 [a0_pred + 月×板块 dummies] OLS 残差化后逐日 Spearman (扣 A0标准TA + 朴素日历/板块 SIGN-R12; 板块由 ts_code 前缀派生 60/00/30/68/other)。**结果** (904,353 OOF 样本 182 日): raw A0=0.10114(t=12.1) / A1张量=-0.00087(t=-0.18 形同零) / A2融合=0.10103(t=11.5 ≈A0 零增量); **残差 rank-IC=-0.00262 (t=-0.605)** 远未达阈值 |IC|>=0.01 且 |t|>=3 → **no_residual**。分regime (SIGN-R11): 动量 +0.011(t=2.22)/反转 -0.012(t=-2.43) 符号相反互抵 = 无稳健残差。**裁决: 关系张量 = 标准TA换皮, 廉价 REJECT, 跳过 RT-005 walk-forward (省~90%算力)**。| 级联 (RT-004 AC#4): prd.json RT-004 passes:true; RT-005 skip:true + verdict REJECT (precondition); RT-006 verdict skipped。| 纪律: SIGN-R02 一次诚实 no_residual 不在同段重调; SIGN-R03 中间IC非落地; SIGN-R04 r20 留 cache/rt004_r20_label.parquet 不进 features (verify leakage 清); SIGN-R05 生产线指纹一致未触碰; SIGN-R08 折级 cache/rt004_fold_{f1,f2,f3}.parquet checkpoint。verify.py EXIT=0 全绿。| 产出: research/rt004_phase1_screen.py + cache/rt004_fold_*.parquet + cache/rt004_r20_label.parquet + verdicts/RT-004.json(no_residual)/RT-005.json(REJECT)/RT-006.json(skipped)。| **下一步: 无 —— 全部 6 task 终态 (RT-001~004 done, RT-005/006 skip), 循环完结。**
