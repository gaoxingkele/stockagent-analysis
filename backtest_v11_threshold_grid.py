"""V11 阈值网格: PUMP_DOWN_EXCL_THRESHOLD ∈ [0.4, 0.5, 0.6, 0.7, 0.8, 1.0(基线)].

目标: 找尾部风险 + α 的最佳平衡点.
- 0.4 太严: 排除多, α 损失大
- 1.0 不过滤 (= v10): α 高但尾部风险大
- 中间最优

双 OOS 验证 (旧 116 日 + 新 50 日).
"""
...
OUT = ROOT / "output" / "backtest_v11_threshold"
...
THRESHOLD_GRID = [0.4, 0.5, 0.6, 0.7, 0.8, 1.0]   # 1.0 = 不过滤 (v10 基线)
...
def build_dual(daily, threshold, oos_start, oos_end):
    rows = []
    for d_, g in daily.groupby("trade_date"):
        ...
        # pump_down 硬过滤 (threshold < 1.0 时生效)
        if threshold < 1.0:
            v7c = v7c[v7c["pump_down_score"] < threshold]
            if len(v7c) == 0: continue
...
def compute_metrics(hold, daily, oos_start, oos_end):
    ...
    return {
        ...
        "avg_n_stocks": pnl["n"].mean(),
    }


def run_oos(label, oos_start, oos_end, daily):
    print(f"\n=== {label}: {oos_start}-{oos_end} ===\n", flush=True)
    print(f"  {'thresh':8s} {'排除 %':8s} {'α':9s} {'Sharpe':8s} {'灾':3s} {'最差':7s} {'avg n':6s}",
           flush=True)
    results = []
    # 计算每个阈值排除比例
    for t in THRESHOLD_GRID:
        if t < 1.0:
            excl_pct = (daily["pump_down_score"] >= t).sum() / len(daily) * 100
        else:
            excl_pct = 0
        hold = build_dual(daily, t, oos_start, oos_end)
        mt = compute_metrics(hold, daily, oos_start, oos_end)
        print(f"  {t:.2f}     {excl_pct:5.1f}    {mt['alpha']:+.3f}pp  {mt['sharpe']:+.2f}    "
               f"{mt['bad_months']}    {mt['worst_month']:+.2f}pp  {mt['avg_n_stocks']:.1f}",
               flush=True)
        results.append({"oos": label, "threshold": t, "excl_pct": excl_pct,
                          **{k: mt[k] for k in
                              ['alpha','sharpe','bad_months','worst_month',
                               'n_hold_days','avg_n_stocks']}})
    return results


def main():
    t0 = time.time()
    print(f"\n=== V11 PUMP_DOWN_EXCL_THRESHOLD 网格 ===\n", flush=True)

    daily = load_window("20251001", "20260522", with_mfk=True)
    ...
    print(f"  pump_up: μ={daily['pump_up_score'].mean():.3f} std={daily['pump_up_score'].std():.3f}",
           flush=True)
    print(f"  pump_down: μ={daily['pump_down_score'].mean():.3f} std={daily['pump_down_score'].std():.3f}",
           flush=True)
    ...
    all_results = []
    all_results.extend(run_oos("旧 OOS", "20251001", "20260331", daily))
    all_results.extend(run_oos("新 OOS", "20260201", "20260515", daily))

    df = pd.DataFrame(all_results)
    df.to_csv(OUT / "threshold_grid.csv", index=False)

    print(f"\n\n=== 双 OOS 平均 (按 threshold 聚合) ===\n", flush=True)
    print(f"  {'thresh':8s} {'avg α':10s} {'avg Sharpe':12s} {'sum 灾':8s} {'avg 最差':10s}",
           flush=True)
    for t in THRESHOLD_GRID:
        sub = df[df["threshold"] == t]
        avg_a = sub["alpha"].mean()
        avg_s = sub["sharpe"].mean()
        sum_b = int(sub["bad_months"].sum())
        avg_w = sub["worst_month"].mean()
        print(f"  {t:.2f}     {avg_a:+.3f}pp  {avg_s:+.3f}       {sum_b}      {avg_w:+.3f}pp",
               flush=True)

    print(f"\n输出: {OUT / 'threshold_grid.csv'}")
