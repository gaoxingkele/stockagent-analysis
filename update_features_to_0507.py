#!/usr/bin/env python3
"""统一增量到 2026-05-07: amount + moneyflow_v1 + mfk + pyramid + v7_extras + regime + regime_extra.

数据源:
  - output/tushare_cache/daily/{YYYYMMDD}.parquet (564 日全市场 OHLC)
  - output/tushare_cache/moneyflow/{YYYYMMDD}.parquet (10 日 04-21 → 05-07 资金流)
...
# 把内部 END 常量推到 05-07, 否则 compute_xxx 内部会硬截到 04-20
_mfk_mod.END = "20260507"
_pyr_mod.END = "20260507"
_v7_mod.END  = "20260507"
_am_mod.END  = "20260507"
...
def merge_moneyflow_bulk_to_cache():
    """把 04-21 → 05-07 的 trade_date-sliced moneyflow append 到单股 cache."""
...
def update_per_stock(daily_by_code):
    ...
    def write(parts, out_dir, name):
        ...
        out_dir.joinpath(f"{name}_ext_0507.parquet").parent.mkdir(exist_ok=True)
        out = out_dir / f"{name}_ext_0507.parquet"
...
def main():
    ...
    print("\n=== Step 3: 单股 5 feature 增量 04-21 → 05-07 ===", flush=True)
